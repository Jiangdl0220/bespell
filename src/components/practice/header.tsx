    ...
    <header style={{background:"var(--card)",borderBottom:"1px solid var(--border)"}}>
      ...
      <div className="max-w-3xl mx-auto px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4 min-w-0">
          <Link href="/" className="text-xs opacity-20 hover:opacity-40 transition-opacity shrink-0">←</Link>
          <div className="min-w-0">
            <h2 className="text-sm font-semibold truncate">{title}</h2>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs opacity-25 tabular-nums">{currentIndex+1}/{total}</span>
        ...
        <motion.div key={combo} initial={{scale:combo>0?1.25:1}} animate={{scale:1}}
          ...
          <span className={`text-2xl font-bold tabular-nums transition-colors ${combo>=10?"text-amber-500":combo>=5?"text-[var(--accent)]":"opacity-15"}`}>
            {combo}
          </span>
        </motion.div>
      </div>
